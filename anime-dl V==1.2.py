...
batch_li = batch(ep_list[from_ep:], max_download)
name_li = batch(range(total_episodes)[from_ep:], max_download)

output_path = mk_dir(name)

for name_b, link_b in zip(name_li, batch_li):
    cmd_li = []
    for n, l in zip(name_b, link_b):
        cmd_li.append(cli_command(n, m3u8(l)))
    for j in cmd_li[1:]:
        os.system(f'start cmd /c {j}')
    os.system(cmd_li[0])
